  ...
  <div class="p-8 max-w-7xl mx-auto animate-fade-in">
    <div class="flex justify-between items-end mb-8">
      <div>
        ...
        <p class="text-gray-400 text-sm">{{ t(`${i18nPrefix}.tooltip`) }}</p>
      </div>
      <div
        class="bg-gray-900 p-1 rounded-lg flex gap-1 border-solid border border-gray-800"
      >
        <button
          id="comm-tab-strategy"
          @click="communityTab = 'strategy'"
          :class="{
            'bg-teal': communityTab === 'strategy',
            'text-gray-400 hover:text-white': communityTab !== 'strategy',
          }"
          class="px-4 py-1.5 text-xs font-bold rounded shadow-sm"
        >
          {{ t(`${i18nPrefix}.tabs.strategy`) }}
        </button>
        <button
          id="comm-tab-screener"
          @click="communityTab = 'screener'"
          :class="{
            'bg-teal': communityTab === 'screener',
            'text-gray-400 hover:text-white': communityTab !== 'screener',
          }"
          class="px-4 py-1.5 text-xs font-bold rounded shadow-sm"
        >
          {{ t(`${i18nPrefix}.tabs.screener`) }}
        </button>
      </div>
    </div>
    ...
    >
      <template v-if="communityTab === 'strategy'">
        <strategyCard v-for="strategy in strategies" :key="strategy.id" :data="strategy" />
      </template>
      <template v-else>
        <screenerCard v-for="screen in screeners" :key="screen.id" :data="screen" />
      </template>
    </div>
  </div>
</template>
...
const i18nPrefix = 'community'

const communityTab = ref("strategy");
    ...
    signals: [
      {
        ...
        entryPrice: "$48,100",
        exitPrice: "$49,050",
        entryTime: "2025-01-05 12:10",
        exitTime: "2025-01-05 16:00",
        pnl: "+1.97%",
        agoMins: 120,
      },
      {
        ...
        entryPrice: "$49,400",
        exitPrice: "$48,600",
        entryTime: "2025-01-04 09:00",
        exitTime: "2025-01-04 12:30",
        pnl: "+1.62%",
        agoMins: 1500,
      },
      {
    ...
    signals: [
      {
        ...
        entryPrice: "$2,420",
        exitPrice: "$2,515",
        entryTime: "2025-01-05 08:00",
        exitTime: "2025-01-05 13:00",
        pnl: "+3.9%",
        agoMins: 200,
      },
      {
...
const openStrategyDetail = (id: string) => {
  router.push(`/strategy-detail?id=${id}`);
};
</script>
