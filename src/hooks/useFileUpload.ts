...
import { SupportUploadFileTypes, TransferMethod } from '@/types'; // 假设这是自定义的类型定义文件
// import { formatFileSize, isAllowedFileExtension, getSupportFileType, uploadRemoteFileInfo, fileUpload, uuid4 } from './utils'; // 假设这是自定义的工具函数文件
...
//     const { files, setFiles } = fileStore.getState();
//     const newFiles = files.map((file) => file.id === newFile.id? newFile : file);
//     setFiles(newFiles);
//   }, [fileStore]);
...
//   const handleDragFileEnter = useCallback((e) => {
//     e.preventDefault();
//     e.stopPropagation();
//     isDragActive.value = true;
//   }, []);

//   const handleDragFileOver = useCallback((e) => {
//     e.preventDefault();
//     e.stopPropagation();
//   }, []);

//   const handleDragFileLeave = useCallback((e) => {
//     e.preventDefault();
//     e.stopPropagation();
