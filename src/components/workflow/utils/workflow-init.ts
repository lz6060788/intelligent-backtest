...
import { correctModelProvider } from '@/utils/index'
...
export const preprocessNodesAndEdges = (nodes: Node[], edges: Edge[]) => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    const currentNode = nodes[i] as Node<LoopNodeType>
  ...
  const newEdges = [...loopNodesWithStartNode].map((nodeItem) => {
    const newNode = newLoopStartNodesMap[nodeItem.id]
    ...
    const parentNode = nodes.find(node => node.id === startNode!.parentNode) || null
    ...
    return {
      ...
      data: {
        ...
        isInLoop,
        loop_id: isInLoop ? startNode!.parentNode : undefined,
...
export const initialNodes = (originNodes: Node[], originEdges: Edge[]) => {
  ...
  const loopNodeMap = nodes.reduce((acc, node) => {
    if (node.parentNode) {
      if (acc[node.parentNode])
        acc[node.parentNode]!.push({ nodeId: node.id, nodeType: node.data!.type })
      else
        acc[node.parentNode] = [{ nodeId: node.id, nodeType: node.data!.type }]
    }
    return acc
  ...
  return nodes.map((node) => {
    ...
    if (node.data!.type === BlockEnum.Loop) {
      ...
      loopNodeData!._children = loopNodeMap[node.id] || []
