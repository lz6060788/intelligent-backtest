...
const toElkNode = (node: Node): ElkNodeShape => ({
  id: node.id,
  width: (node.width as number) ?? DEFAULT_NODE_WIDTH,
  height: (node.height as number) ?? DEFAULT_NODE_HEIGHT,
})
...
const buildIfElseWithPorts = (
  ...
  return {
    node: {
      id: ifElseNode.id,
      width: (ifElseNode.width as number) ?? DEFAULT_NODE_WIDTH,
      height: (ifElseNode.height as number) ?? DEFAULT_NODE_HEIGHT,
      ports,
      layoutOptions: {
        'elk.portConstraints': 'FIXED_ORDER',
      },
...
export const getLayoutByDagre = async (originNodes: GraphNode[], originEdges: GraphEdge[]): Promise<LayoutResult> => {
  edgeCounter = 0
  const nodes = cloneDeep(originNodes).filter(node => !node.parentNode && node.type === CUSTOM_NODE)
...
export const getLayoutForChildNodes = async (
  ...
  edgeCounter = 0
  const nodes = cloneDeep(originNodes).filter(node => node.parentNode === parentNodeId)
  if (!nodes.length)
    return null
