<template>
  <div v-if="outputVars.length > 0" class='mb-1 py-1 w-60'>
    <div class='flex justify-start rounded-md p-1'>
      <div class='w-12 h-4 shrink-0 items-center rounded px-1 text-xs font-semibold uppercase text-text-secondary mr-1 truncate'>{{ t(`${i18nPrefix}.output`) }}</div>
...
import { BlockEnum, VarType, type NodePanelProps } from '@/types';
